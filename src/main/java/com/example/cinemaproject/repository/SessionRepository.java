...
public interface SessionRepository extends JpaRepository<Session, Long> {
    ...
    @Query("SELECT s.movie.title FROM Session s WHERE s.room.id = :roomId AND ((:startTime BETWEEN s.startTime AND s.endTime) OR (:endTime BETWEEN s.startTime AND s.endTime))")
    String getMovieByRoomAndTime(@Param("roomId") Long roomId, @Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);
